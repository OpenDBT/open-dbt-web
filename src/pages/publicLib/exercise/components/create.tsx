...
import { getSceneNameList, updateExercise, testRunAnswer } from '@/services/teacher/course/publicLib';
...
const addIndex = (props: any) => {
  ...
  const [selectSceneId, setSelectSceneId] = useState<number>(-1);//选中场景的id
  const [viewSceneDetailDisabled, setViewSceneDetailDisabled] = useState<boolean>(true);//查看场景详情是否禁用
  const [resultSetModalVisible, setResultSetModalVisible] = useState<boolean>(false);//结果集弹窗是否显示
