...
import { getExamStudentReportCard, getExamDetailByExamClassId, getStudentExerciseReportCard }
  from '@/services/teacher/course/exam';
...
const ExamReport = (props: any) => {
  ...
  const openStuAnswerSituationModal = (record: API.ExamStudentReportCard) => {
    getStudentExerciseReportCard(examClassId, record.studentId).then((result) => {
      if (result.success) {
        ...
        // setStuName(record.studentName);
        // setStuCode(record.studentCode);
        setmodalParam({ name: record.studentName, code: record.studentCode, examClassId: examClassId, courseId: courseId });
  ...
  const [modalParam, setmodalParam] = useState<{ name: string, code: string, examClassId: number, courseId: number }>();
