...
import { updateExercise, getExerciseById } from '@/services/teacher/course/exercise';
...
const updateIndex = (props: any) => {
  ...
  useEffect(() => {
    ...
    getExerciseById(exerciseId).then((result) => {
      ...
      setKnowledgeIds(result.obj.knowledgeIds)
   ...
   * 保存
   */
  const onFinish = async (fieldsValues: API.ExerciseRecord) => {
    console.log('fieldsValues', fieldsValues)
    fieldsValues.exerciseDesc = fieldsValues.exerciseDesc.toHTML();
    const result = await updateExercise({ ...fieldsValues, knowledgeIds: knowledgeIds })
    if (result.success) {
